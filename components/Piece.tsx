...
export const Piece: React.FC<PieceProps> = ({ piece, padding, stepSize, isSelected, isActiveSide, onSelect }) => {
  ...
  // 棋子底座：象牙色棋面 + 阵营色外圈
  const rimColor = isRed ? '#b91c1c' : '#292524';
  const rimDark = isRed ? '#7f1d1d' : '#0c0a09';
  ...
  return (
    ...
    >
      <div
        ...
        className={`piece-token w-full h-full transition-all duration-200
          ${isActiveSide ? 'cursor-pointer' : 'cursor-default'}
          ${isSelected ? 'piece-selected' : isActiveSide ? 'piece-active' : ''}
        `}
      >
        <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
          <defs>
            <radialGradient id={`face-${gid}`} cx="0.38" cy="0.32" r="0.75">
              <stop offset="0%" stopColor="#fffaf0" />
              <stop offset="70%" stopColor="#f5e3c3" />
              <stop offset="100%" stopColor="#e3c79a" />
            </radialGradient>
          ...
          {isSelected && (
            <>
              <circle cx="50" cy="50" r="57" fill="rgba(250, 204, 21, 0.22)" />
              <circle cx="50" cy="50" r="51" fill="none" stroke="#facc15" strokeWidth="3" />
              <circle
                cx="50" cy="50" r="58"
          ...
          {/* 棋子厚度（侧面） */}
          <circle cx="50" cy="54" r="46" fill={rimDark} />
          {/* 外圈 */}
          <circle cx="50" cy="50" r="46" fill={`url(#rim-${gid})`} />
          {/* 棋面 */}
          <circle cx="50" cy="50" r="40" fill={`url(#face-${gid})`} />
          <circle cx="50" cy="50" r="36" fill="none" stroke={rimColor} strokeWidth="1.2" opacity="0.45" />
          {/* 外圈高光 */}
          <path d="M16 38 A36 36 0 0 1 62 8" fill="none" stroke="#ffffff" strokeWidth="2.5" strokeLinecap="round" opacity="0.28" />

          {/* 坦克（缩放后置于棋面中央，只有坦克随阵营转向） */}
          <g transform={`translate(50 50) scale(0.68) rotate(${rotation}) translate(-50 -50)`}>
            {/* 履带 */}
